...
New general purpose buckets block new SSE-C writes by default, while existing SSE-C objects remain readable with the correct key.
...
Every bucket has an effective default of SSE-S3.

`GetBucketEncryption` returns that effective default even when the operator has not selected a different algorithm.
...
- Bucket Keys only with `aws:kms`, never with `AES256` or `aws:kms:dsse`, and only where the selected provider implements their semantics.
...
One layer uses the selected KMS key provider.

The second layer uses the gateway-managed SSE-S3 key provider.

When both layers use local key material, they use distinct wrapping-key IDs or key rings in addition to independent data keys and nonces.

The implementation does not reuse a nonce, data key, or wrapped-key record between layers.

### SSE-C

SSE-C is accepted only over TLS.
...
| Policy and logging | `auth/`, `s3api/middlewares/debuglogger.go` | Encryption condition keys and complete redaction tests |
...
- Reject insecure SSE-C transport before reading object payload bytes.
...
Buckets created after the feature is enabled start with SSE-C writes blocked, matching the April 2026 Amazon S3 default.
...
3. Enable Encryption capability in audit mode and inventory legacy objects.
